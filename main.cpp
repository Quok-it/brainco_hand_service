...
void update_finger(DeviceHandler* handle, uint8_t slave_id,
    ...
    TouchFingerData* td = modbus_get_touch_status(handle, slave_id);
      if (td) {
          for (int i = 0; i < 5; ++i) {
...
void hand_worker(DeviceHandler* handle, uint8_t slave_id, const std::string& ns) {
    ...
    auto touchstate = std::make_unique<unitree::robot::RealTimePublisher<unitree_go::msg::dds_::MotorStates_>>("rt/brainco/" +
  ns + "/touch");
